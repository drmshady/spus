...
Original file is located at
    https://colab.research.google.com/drive/1x5SJSyZFcmcTjMNRG-Px2MP_J16IXCBj
"""
...
# --- ⭐️ دالة جديدة: التنسيق الشرطي المتقدم للجداول ⭐️ ---
def apply_comprehensive_styling(df):
    ...
    text_style_cols = [col for col in ['Valuation (Graham)', 'MACD_Signal', 'Price vs. Levels'] if col in cols_to_show]
    ...
    # تحويل الأعمدة الرقمية (التي قد تكون كنص) إلى أرقام لتطبيق التدرج
    ...
    if 'Risk % (to Support)' in df_display.columns:
        # تدرج عكسي (الأخضر للمخاطر الأقل)
...
# --- ⭐️ دالة جديدة: العثور على أحدث التقارير ⭐️ ---
def get_latest_reports(excel_base_path):
    """
    يبحث عن أحدث ملف إكسل وملف PDF تم إنشاؤهما.
    """
...
def main():
    ...
    # المسار الكامل لملف الإكسل الأساسي
    ...
    if data_sheets is None:
        ...
        for i, sheet_name in enumerate(tab_titles):
            with tabs[i]:
                ...
                # --- الرسوم البيانية (Charts) - بدون تغيير ---
                ...
                # --- ⭐️ تطبيق التنسيق المتقدم الجديد ⭐️ ---
                # استبدال 'st.dataframe(style_dataframe_text_only(df_to_show)...)'
                styled_df = apply_comprehensive_styling(df_to_show)
                st.dataframe(styled_df, use_container_width=True)
                # --- ⭐️ نهاية التنسيق المتقدم ⭐️ ---


                # زر تنزيل CSV الخاص بالتاب (Sheet) الحالية - بدون تغيير

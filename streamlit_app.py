...
def style_dataframe_text_only(df):
    ...
    if not style_cols:
        return df.style # إرجاع الاستايل الأصلي إذا لم تكن هناك أعمدة لتنسيقها
    ...
    if data_sheets is None:
        ...
        for i, sheet_name in enumerate(tab_titles):
            with tabs[i]:
                ...
                # The broken code block has been removed and replaced with this:

                # 1. Apply the styling function you defined
                styled_df = style_dataframe_text_only(df_to_show)
                st.dataframe(styled_df, use_container_width=True)

                # 2. Correctly indent the download button to be inside the loop
                ...
                st.download_button(
                    ...
                    mime='text/csv',
                    key=f"csv_download_{sheet_name}" # Added a key for stability
                )
                # --- ⭐️⭐️⭐️ END FIX ⭐️⭐️⭐️ ---

...
# (هذه هي المكتبات التي كان يستخدمها البلوك الرئيسي في spus.py)
import logging
from concurrent.futures import ThreadPoolExecutor, as_completed
import openpyxl
from openpyxl.styles import Font

# (محاولة استيراد مكتبة إنشاء التقارير)
try:
...
@st.cache_data  # استخدام الكاش الخاص بـ Streamlit
def load_excel_data(excel_path):
    ...
    try:
        ...
        for sheet in sheet_names:
            # قراءة البيانات مع تحديد العمود الأول كـ index
...
def run_full_analysis(CONFIG):
    ...
    """
    # إعداد واجهة Streamlit لإظهار التقدم
    progress_bar = st.progress(0, text="Starting analysis...")
    status_text = st.empty()
    status_text.info("يتم الآن بدء التحليل...")

    # --- بداية الكود المنسوخ من spus.py ---
    MAX_RISK_USD = 50  #

    # (إعداد اللوجر الأساسي - سيبقى للعمل في الخلفية)
    logging.basicConfig(
        level=logging.INFO,
    ...
    progress_bar.progress(0.9, text="Calculating Risk/Reward...") # تقدير
    ...
    for ticker in tickers_to_report: #
        ...
        result_data = { #
            ...
            'Dividend Yield (%)': fin_info.get('Dividend Yield'), # تم التأكد من وجوده
            '1-Year Momentum (%)': momentum_data.get(ticker, "N/A"),
            'Return on Equity (ROE)': fin_info.get('Return on Equity (ROE)'),
            # ... (إضافة أي أعمدة أخرى مطلوبة لـ Z-Score)
        }
    ...
    if REPORTLAB_AVAILABLE:
        try:
            ...
            # (نسخ دالة create_pdf_table من spus.py)
            ...
            def create_pdf_table(title, df): #
                ...
                # (... كل لوجيك اختيار الأعمدة من spus.py ...)
                ...
                table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.green), ('GRID', (0, 0), (-1, -1), 1, colors.black)])) #
                elements = [Paragraph(title, styles['h2']), Spacer(1, 0.1*inch), table, Spacer(1, 0.25*inch)] #
                return elements

            # (إضافة الجداول إلى PDF)
...
def main():
    ...
    if data_sheets is None:
        ...
        # إنشاء Tabs (علامات تبويب) لكل شيت في الإكسل
        tab_titles = list(data_sheets.keys())
        # إزالة "All Results" وجعله آخر تاب

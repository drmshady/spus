...
Original file is located at
    https://colab.research.google.com/drive/175PonR7Nc8EVaVO7AdmE3Fnu5S9OSc7r
"""
...
def style_dataframe_text_only(df):
    ...
    يتم الآن إضافة تنسيق تدرج (Gradient) للأعمدة الرقمية الرئيسية.
    """
    df_styled = df.style

    # 1. التنسيق النصي (Critical signals)
    def highlight_text(val):
        val_str = str(val).lower()
        if 'undervalued' in val_str or 'bullish crossover' in val_str:
            return 'color: #00A600; font-weight: bold;' # Dark Green
        elif 'overvalued' in val_str or 'bearish' in val_str:
            return 'color: #D30000; font-weight: bold;' # Dark Red
        elif 'near support' in val_str:
            return 'color: #004FB0; font-weight: bold;' # Dark Blue
        return ''

    text_cols = [col for col in ['Valuation (Graham)', 'MACD_Signal', 'Price vs. Levels'] if col in df.columns]

    if text_cols:
        df_styled = df_styled.apply(lambda x: x.map(highlight_text), subset=text_cols)

    # 2. التنسيق الرقمي (Quant Score & R/R Ratio) - يتطلب تحويل الأعمدة مؤقتاً
    quant_col = 'Final Quant Score'
    rr_col = 'Risk/Reward Ratio'

    # Create a numeric copy to apply the gradient, as dataframes are stored as strings
    df_numeric = df.copy()

    if quant_col in df_numeric.columns:
        df_numeric[quant_col] = pd.to_numeric(df_numeric[quant_col], errors='coerce')
    ...
    if rr_col in df_numeric.columns:
        df_numeric[rr_col] = pd.to_numeric(df_numeric[rr_col], errors='coerce')
        # Apply Blue gradient
        df_styled = df_styled.background_gradient(
            cmap='Blues',
            subset=[rr_col],
    ...
    if REPORTLAB_AVAILABLE:
        try:
            ...
            def create_pdf_table(title, df):
                ...
                # ⭐️ UPDATED cols_map for concise output ⭐️
                cols_map = {
                    ...
                    'Top 20 by Final Quant Score': (['Ticker', 'Final Quant Score', 'Risk/Reward Ratio', 'Last Price', 'Trend (50/200 Day MA)', 'MACD_Signal', 'Cut Loss Level (Support)', 'Fib 161.8% Target', 'Shares to Buy ($50 Risk)', 'Risk % (to Support)'], ['Ticker', 'Score', 'R/R', 'Price', 'Trend', 'MACD', 'Stop Loss', 'Fib Target', 'Shares', 'Risk %']),

                    'Top Quant & High R-R (Ratio > 1)': (['Ticker', 'Final Quant Score', 'Risk/Reward Ratio', 'Last Price', 'Trend (50/200 Day MA)', 'Cut Loss Level (Support)', 'Fib 161.8% Target', 'Shares to Buy ($50 Risk)', 'Risk % (to Support)'], ['Ticker', 'Score', 'R/R', 'Price', 'Trend', 'Stop Loss', 'Fib Target', 'Shares', 'Risk %']),

                    'Top 10 Undervalued (Graham)': (['Ticker', 'Final Quant Score', 'Last Price', 'Fair Price (Graham)', 'Valuation (Graham)', 'Risk/Reward Ratio', 'Cut Loss Level (Support)', 'Fib 161.8% Target', 'Dividend Yield (%)'], ['Ticker', 'Score', 'Price', 'Graham Price', 'Valuation', 'R/R', 'Stop Loss', 'Fib Target', 'Div %']),

                    'New Bullish Crossovers (MACD)': (['Ticker', 'Final Quant Score', 'MACD_Signal', 'Last Price', 'Trend (50/200 Day MA)', 'Risk/Reward Ratio', 'Cut Loss Level (Support)', 'Fib 161.8% Target', 'Shares to Buy ($50 Risk)'], ['Ticker', 'Score', 'MACD', 'Price', 'Trend', 'R/R', 'Stop Loss', 'Fib Target', 'Shares']),

                    'Stocks Currently Near Support': (['Ticker', 'Final Quant Score', 'Price vs. Levels', 'Last Price', 'Risk % (to Support)', 'Risk/Reward Ratio', 'Cut Loss Level (Support)', 'Fib 161.8% Target', 'Shares to Buy ($50 Risk)', 'Next Earnings Date'], ['Ticker', 'Score', 'vs. Levels', 'Price', 'Risk %', 'R/R', 'Stop Loss', 'Fib Target', 'Shares', 'Earnings'])
                }
